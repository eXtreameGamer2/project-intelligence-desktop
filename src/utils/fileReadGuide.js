...
Spreadsheets (CSV, TSV, XLSX, XLSM, XLS, ODS): each RECORD n is one spreadsheet row. The name before each colon is the column header. Empty cells are omitted. Do not invent rows or assume a meaning for a column that is not named. Sheets, if present, are named with SHEET.
...
export function fileTypeHowToRead(fileType) {
  switch (String(fileType || '').toLowerCase()) {
    case 'csv':
    case 'tsv':
      return 'Each RECORD is one spreadsheet row. The name before each colon is the column header. Skip empty fields. A field like (=SUM(A1:A3)) [text formula] is formula text, not a calculated number. Do not invent rows.';
    case 'xlsx':
    case 'xlsm':
      return 'Sheets are named. Each RECORD is one row in that sheet. The name before each colon is the column header. A field like 42 (=SUM(B2:B5); B2=10, B3=12) is Excel\'s saved result plus the formula. Use the saved result; do not recalculate or invent missing refs.';
    case 'xls':
    case 'ods':
      return 'Sheets are named. Each RECORD is one row in that sheet. The name before each colon is the column header. Skip empty fields. Do not invent rows.';
    case 'json':
